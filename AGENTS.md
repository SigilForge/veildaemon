...
- **Canonical Unified Push Command**: Use `npm run push` (or `npm run ship` / `node scripts/unified-push.mjs`) to execute pre-flight validation, push to GitHub (`origin main`), deploy VeilLink to Vercel, and handle Relay/API Vercel deployments and Supabase verification sync in one command.
...
- **Vercel project `veildaemon`** (`prj_7bgZ4yTaZOd5QsR6WOpXElo2pbpv`, `api.veildaemon.app`) serves repo-root API functions (`api/*`, root `vercel.json`) used by reports, alerts, routing, etc. This is the project whose deployment status GitHub shows as a commit/PR check.
- **Vercel project `veillink`** (`prj_yIporTovuVLyKTbvfPtxGi6uwuiQ`, aliases `app.veildaemon.app` and `go.veildaemon.app`) is a **completely separate** Vercel project from the one above, deployed from `veillink/`. **A green "Vercel" check on a commit/PR is the root `veildaemon` project's status, not this one** — it is not evidence that VeilLink has been deployed. Deploy VeilLink only via `npm run veillink:deploy` (or `npm run push`, which now calls that same guarded path) — never a raw `cd veillink && vercel --prod --yes`. That command (`scripts/deploy-veillink.mjs`) refuses to deploy if `veillink/.vercel/project.json` is not linked to this exact project id, and performs its own post-deploy verification (project, target, aliases, live route) rather than trusting the root project's CI status. If you're ever unsure which project is linked, run `npm run veillink:deploy:dry-run` first.
...
- Do not add one-dimensional barcodes to stream overlays or broadcast backplates by default. The BRB overlay is the explicit exception: it may use real one-dimensional barcodes when they encode a real destination, match the BRB palette and frame geometry, and do not cover operational overlay content. A composition may use at most one deliberate, phone-readable QR code when it has a real destination, a designed placement, and does not cover operational overlay content. QR/barcode replacement rules for print, download, or public conversion assets must otherwise exclude `stream/`.
