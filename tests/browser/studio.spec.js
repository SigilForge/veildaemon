  ...
  test("RelayDaemon requests one character package and uses complete platform summaries", async ({ page }) => {
    ...
    await page.route("**/api/character", async (route) => {
      if (route.request().method() === "GET") {
        await route.fulfill({ contentType: "application/json", body: JSON.stringify({ status: "ok", engine: "ollama", model: "hf.co/zerofata/MS3.2-PaintedFantasy-v4.1-24B-GGUF:Q5_K_M", thinking: "off" }) });
        return;
      }
      requestNumber += 1;
      ...
      await route.fulfill({ contentType: "application/json", body: JSON.stringify({ status: "ok", engine: "ollama", model: "hf.co/zerofata/MS3.2-PaintedFantasy-v4.1-24B-GGUF:Q5_K_M", thinking: "off", result: { masterDraft: "Everybody wants the clean answer. Cute. Hunger is honest; it does not hide behind a tidy form. Sorry. That was supposed to be funny. The room can help people and still leave teeth marks.", platformDrafts: { x: "Everybody wants the room to be either helpful or dangerous. Cute. It helps people, and the teeth marks still count; both truths belong in the same answer.", threads: "Everybody wants the room to be either helpful or dangerous. Cute. It helps people, and the teeth marks still count; both truths belong in the same answer.", bluesky: requestNumber === 2 ? "The room helps people, but the hunger feels like." : "The room helps people, but the teeth marks still count. Hunger does not become harmless just because it is honest. Sorry. That was meant to be reassuring.", mastodon: "Everybody wants a clean answer about the room: helpful or dangerous, refuge or appetite. Cute. It helps people, and the teeth marks still count. Hunger is honest, but honesty does not make it harmless. Sorry. That was supposed to make the contradiction easier to hold." }, platformReviews: { x: { completeThought: true, characterCount: 148 }, threads: { completeThought: true, characterCount: 148 }, bluesky: { completeThought: requestNumber !== 2, characterCount: requestNumber === 2 ? 48 : 148 }, mastodon: { completeThought: true, characterCount: 259 } }, validation: { voiceMatch: 0.9, sourceFidelity: 0.92, canonSafe: true, knowledgeBoundarySafe: true, characterMarkers: ["personal hunger framing", "humor-to-sincerity pivot"], warnings: [] } } }) });
    });
    await page.goto("/studio/relay/");
