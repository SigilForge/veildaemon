...
test("operator imports Handler authorization unlock packets", async ({ page }) => {
  ...
  await expect(page.locator("#presentation-readout-body")).toContainText(/Misfire fills Blood Load/);
  await expect(page.locator("#misfire-severity-wrap")).toBeVisible();
  const bloodPip = page.locator(".line-tracker.blood_load .pip").first();
  const pipBox = await bloodPip.boundingBox();
  expect(pipBox?.width || 0).toBeGreaterThan(8);
  expect(pipBox?.height || 0).toBeGreaterThan(6);
  ...
  await expect(page.locator(".line-tracker").filter({ hasText: "Blood Load" }).locator(".pip-derived")).toHaveText("Starving");
  await expect(page.locator("#presentation-readout-body")).toContainText(/Warm enough/);
  await page.locator(".line-tracker").filter({ hasText: "Blood Load" }).locator(".pip").nth(2).click();
  await expect(page.locator(".line-tracker").filter({ hasText: "Blood Load" }).locator(".pip-derived")).toHaveText("Coherent");
  await expect(page.locator('[name="sanguineSaturated"]')).toHaveCount(0);
  await page.getByLabel("Observed Symptom").fill("The room learned my hunger before I did.");
  await page.locator(".misfire-severity").getByRole("button", { name: "Severe (+2)" }).click();
  await expect(page.locator(".line-tracker").filter({ hasText: "Blood Load" }).getByText("5/6")).toBeVisible();
  await expect(page.locator(".line-tracker").filter({ hasText: "Blood Load" }).locator(".pip-derived")).toHaveText("Predatory Saturation");
  await expect(page.locator("#misfire-presentation-load-note")).toContainText("Blood Load rises by 2");
  await expect(page.locator("#misfire-presentation-load-note")).toContainText("warmer than the room");
