...
test("operator imports Handler authorization unlock packets", async ({ page }) => {
  ...
  await page.locator("#presentation-readout-layer").evaluate((el) => { el.open = true; });
  await expect(page.locator("#presentation-readout-body")).toContainText(/Blood Load [23]\/6, Coherent/);
  await expect(page.getByText(/Rain-gauge fill/)).toBeVisible();
  ...
  await expect(page.locator("#presentation-readout-layer")).toHaveAttribute("open", "");
  await expect(page.getByText(/Cue: Blood load is still too low/)).toBeVisible();
  ...
  await expect(page.locator("#presentation-readout-body")).toContainText("Blood Load 4/6, Saturated");
