"""Packs integration: select and apply Persona/Logic/AR packs.

Reads selection from veil.config [packs] section; falls back to first of each type.
Applies:
- Persona: build a persona dict compatible with persona_injector
- Logic: register simple HRM custom actions from actions[] entries
- AR: validate token via license gate; expose first model asset path
"""
...
try:
    ...
    try:
        ...
        sd = _Path(__file__).parent / 'StreamDaemon'
...
def apply_logic_pack(engine: Any, p: Pack) -> None:
    ...
    for idx, act in enumerate(actions):
        ...
        def _make_action(text=vline, gly=glyph):
            def _fn(ctx):
                # The engine.hrm.emit_actions will speak when action returns lines
                out = []
                if gly:
                    try:
                        # Represent a glyph action object minimally
...
def resolve_ar_model_url(ar_pack: Optional[Pack]) -> Optional[str]:
    ...
    if not models:
        return None
    # We assume models are made available under StreamDaemon/assets; use basename
