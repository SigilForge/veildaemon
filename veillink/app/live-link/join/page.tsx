...
export const metadata: Metadata = buildMetadata({
  title: "Join Cell",
  description: "Join a live Cell with your Operator file.",
  path: "/live-link/join",
  noIndex: true,
});
...
export default async function TableJoinPage({ searchParams }: Props) {
  ...
  await requireUser().catch(() =>
    redirect(`/login?next=${encodeURIComponent(`/live-link/join?code=${q.code || ""}`)}`),
  );
  return (
    <main className="page table-page">
      <p className="eyebrow">Operator join</p>
      <h1 className="page-title">Join Cell {q.code ? `· ${q.code}` : ""}</h1>
      <p className="lede">Select your Operator file and enter the Cell Code. Access is temporary and revocable.</p>
