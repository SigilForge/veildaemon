...
- Writer -> editor -> ruler. The story model writes; the editor model reshapes without changing meaning; the runtime judges. No model is trusted to count characters or to grade its own work.
  ...
  - Required concepts are two writer-declared groups, the two halves of the central thought: `whatChanges` and `whyItMatters`, each 1-3 short anchors. The runtime keeps only anchors grounded in the master (every word present); a group left empty is a writer failure retried by the writer's ladder. Every constrained lane must contain at least one anchor from each group (an anchor phrase matches on its head noun); a lane missing a group goes to the editor as `concept_dropped` with `missingGroups`, and the retry names the missing half and its anchors. No model call classifies concepts; never read from a fixture.
