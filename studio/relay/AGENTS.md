...
- Residency (`RELAY_MODEL_RESIDENCY`, default `adaptive`). Relay owns exactly its two models and never unloads or manipulates another process's model; it may observe foreign residency (free VRAM via `nvidia-smi`, `/api/ps`) only for its own scheduling (VeilForge: residency is not ownership). Adaptive: the writer is released before the editor only when the editor would not fit (`RELAY_EDITOR_VRAM_MB`, unknown counts as not fitting); the editor is released when its stage ends; after the response is sent the writer is rewarmed in the background, load-only, with a bounded idle TTL (`RELAY_WRITER_WARM_TTL`, default 10m), only when it is not resident and headroom is safe (`RELAY_WRITER_VRAM_MB`); a new request joins an in-flight rewarm. `release` = always cold writer; `keep` = both resident.
- Every Ollama call records `load_duration`, `prompt_eval_duration`, and `eval_duration` (ms, numbers only) in the response `timings` and the acceptance artifact.
- A cold start is a supported state: the page's generation timeout is 360 s (observed cold ceiling 282 s plus margin); the acceptance runner's direct client and UI wait allow 420 s. Timeouts are headroom for a legitimate cold start, not the residency strategy.
