...
- Writer -> editor -> ruler. The story model writes; the editor model reshapes without changing meaning; the runtime judges. No model is trusted to count characters or to grade its own work.
  ...
  - The writer's groups are editing hints, not authority. The verifier's existing call also returns positive `halves` evidence for every candidate (change = what is happening or lost; impact = why it matters), each as a source quote and a rewrite quote. The runtime counts a half as preserved only when both quotes are grounded; a missing or ungrounded half fails closed as `change_missing` / `impact_missing`. No extra model call.
