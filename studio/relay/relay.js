(function () {
  ...
  const platformConfig = [
    // X Premium long posts: limit comes from the shared platform policy (studio/relay/platform-policy.js).
    { id: "x", name: "X", target: "Full long copy · first 280 show", limit: window.RelayPlatformPolicy.platforms.x.max, crop: "16:9 or 1.91:1 · center-safe", defaultOn: true },
  ...
  // Platform targets come from studio/relay/platform-policy.js, the same map the local bridge enforces.
  function relayPolicyPromptLines() {
    const policy = window.RelayPlatformPolicy;
    if (!policy || typeof policy.promptLines !== "function") throw new Error("Relay platform policy did not load");
    return policy.promptLines();
  }
  ...
  function generateCopy(config, text, analysis) {
    ...
    switch (config.id) {
      case "x":
        // Long-form lane (X Premium): full context body, not a 240-character teaser.
        return clip(joinParts([voicedHook, contextBody, ctaLine, tagsFor("x", analysis, 3)]), config.limit);
