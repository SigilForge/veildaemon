...
function entryPage(record) { return `<!doctype html>\n<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(record.title)} | Cradlepoint Studio</title><meta name="description" content="${esc(record.summary)}"><link rel="canonical" href="https://veildaemon.app${record.canonicalUrl}"><meta property="og:title" content="${esc(record.title)}"><meta property="og:description" content="${esc(record.summary)}"><meta property="og:url" content="https://veildaemon.app${record.canonicalUrl}"><link rel="stylesheet" href="/studio/studio.css?v=20260715-relay2"></head><body class="subpage page-shell"><a class="skip-link" href="#main">Skip to content</a><header class="site-header"><a class="brand" href="/studio/"><img src="/studio/assets/brand/cradlepoint-studio-emblem-256.webp" alt="Cradlepoint Studio"><span><strong>CRADLEPOINT</strong><small>STUDIO</small></span></a><nav aria-label="Primary navigation"><a href="/studio/projects/">Projects</a><a href="/studio/technology/">Technology</a><a href="/studio/news/">News</a><a href="/studio/relay/">RelayDaemon</a></nav></header><main id="main"><article class="sub-hero text-only"><div><p class="eyebrow">${esc(record.category || "Studio news")} · ${esc(record.realityLayer)}</p><h1>${esc(record.title)}</h1><p>${esc(record.summary)}</p><p><small>Published ${esc(new Date(record.publishedAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }))} · ${esc(record.voice)}</small></p></div></article><section class="section-block paper"><div class="notice-copy">${esc(record.body).split("\n\n").map((paragraph) => `<p>${paragraph.replace(/\n/g, "<br>")}</p>`).join("")}</div></section></main></body></html>\n`; }
