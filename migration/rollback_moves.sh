...
if command -v jq >/dev/null 2>&1; then
  echo "Rolling back move plan using jq..."
  # rollback in reverse order to avoid path conflicts
  mapfile -t lines < <(jq -r '.[] | [.src, .dst] | @tsv' "$PLAN")
  for (( idx=${#lines[@]}-1 ; idx>=0 ; idx-- )); do
    IFS=$'\t' read -r SRC DST <<<"${lines[$idx]}"
    echo "-- $DST -> $SRC"
...
echo "Rollback applied. Review changes with 'git status'."
