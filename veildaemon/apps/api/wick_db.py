...
def get_recent_events(n=10):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT timestamp, wicks, delta, reason, context FROM wick_events ORDER BY id DESC LIMIT ?",
        (n,),
    )
    events = c.fetchall()
    conn.close()
...
def get_today_events():
    ...
    c = conn.cursor()
    c.execute(
        "SELECT timestamp, wicks, delta, reason, context FROM wick_events WHERE timestamp >= ? ORDER BY id ASC",
        (midnight,),
    )
    events = c.fetchall()
    conn.close()
...
def get_mood_history(days=30):
    ...
    c.execute(
        "SELECT timestamp, mood FROM journal_entries WHERE timestamp >= ? ORDER BY timestamp ASC",
        (cutoff,),
    )
    data = c.fetchall()
    conn.close()
