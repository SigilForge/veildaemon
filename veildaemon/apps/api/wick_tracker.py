...
def decay_wicks(decay_amount=DEFAULT_DECAY_AMOUNT, interval=DEFAULT_DECAY_INTERVAL):
    ...
    if decays > 0:
        ...
        save_state(state)
        log_wick_event(-total_decay, f"auto-decay ({decays}x @ {decay_amount}/interval)")
    return state["wicks"]
...
def get_fit_service():
    ...
    if client_cfg:
        try:
            ...
            print(
                f"⚠️ Failed to load encrypted Google client config: {e}. Falling back to client_secret.json"
            )
...
def get_today_stats(service):
    ...
    start_time = int(datetime.datetime(now.year, now.month, now.day).timestamp() * 1000)
    ...
    step_resp = (
        ...
        .dataSources()
        .datasets()
        .get(userId="me", dataSourceId=step_ds, datasetId=dataset)
        .execute()
    )
    ...
    hr_resp = (
        ...
        .dataSources()
        .datasets()
        .get(userId="me", dataSourceId=hr_ds, datasetId=dataset)
        .execute()
    )
    ...
    try:
        stress_resp = (
            ...
            .dataSources()
            .datasets()
            .get(userId="me", dataSourceId=stress_ds, datasetId=dataset)
            .execute()
        )
    ...
    sleep_resp = (
        ...
        .dataSources()
        .datasets()
        .get(userId="me", dataSourceId=sleep_ds, datasetId=dataset)
        .execute()
    )
