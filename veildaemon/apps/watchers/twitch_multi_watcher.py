...
def discover_channels(min_viewers: int = 75, limit: int = 10, language: str | None = None, max_viewers: int | None = 250) -> list[str]:
	params = {"first": "100"}
	if language:
	...
	try:
		...
		for s in streams:
			...
			in_range = isinstance(vc, int) and vc >= min_viewers and (max_viewers is None or vc <= max_viewers)
			...
			for line in data.split("\r\n"):
				...
				if line.startswith("PING"):
					...
					if debug:
						print("[DBG] PING -> PONG")
					continue
				if debug and ("JOIN" in line or "ROOMSTATE" in line or "USERSTATE" in line or "GLOBALUSERSTATE" in line):
					print("[DBG] " + line[:180])
				if " PRIVMSG #" in line:
					try:
...
def watch_channels(debug: bool = False, min_viewers: int = 75, limit: int = 10, language: str | None = None, max_viewers: int | None = 250, inactivity_seconds: int = 45, rediscover_on_inactive: bool = True):
	targets = discover_channels(min_viewers=min_viewers, limit=limit, language=language, max_viewers=max_viewers)
	if debug:
	...
	if rediscover_on_inactive and not log:
		targets2 = discover_channels(min_viewers=min_viewers, limit=limit, language=language, max_viewers=max_viewers)
		if debug:
	...
	if log:
		...
		print(f"Saved {len(log)} messages from {len(set(x['channel'] for x in log))} channels to {LOG_PATH.name}")
...
if __name__ == "__main__":
	...
	ap.add_argument("--max-viewers", type=int, default=250, help="Only consider streams with viewers <= this number (use 0 to disable)")
	ap.add_argument("--inactive-seconds", type=int, default=45, help="Skip a channel if no chat for this many seconds")
	# Enabled by default; provide a flag to disable
	ap.add_argument("--no-rediscover-on-inactive", dest="rediscover_on_inactive", action="store_false", help="Disable re-discovery on inactivity")
	...
	watch_channels(
		...
		max_viewers=(None if (hasattr(args, 'max_viewers') and args.max_viewers == 0) else args.max_viewers),
