...
2. **`api/`** — Vercel serverless functions (repo root, `api.veildaemon.app`) for reports, alerts, routing, Twitch EventSub, character/scan-code proxying, Book One claims, and the Cell/Live-Link Bearer-token API (`api/cell/[action].js` + `lib/cellSync.js`) that connects the real Operator and Handler apps live across devices.
3. **`veillink/`** — a separate Next.js 16 + Supabase app (`app.veildaemon.app`) providing authenticated identity, Dynamic Links/QR, and billing. Has its own `package.json`, deploys as an isolated Vercel project. It no longer hosts a Cell/Live-Link gameplay UI (that lived at `/live-link` as a disconnected mini-sheet requiring manual export/import; removed once `operator/` and `handler/` grew their own Connect UI backed by `cell-sync-remote.js`) — its role for Cell/Live-Link now is purely issuing the Supabase session those apps authenticate with.
...
Governed separately by `studio/relay/AGENTS.md` — read it and `tests/fixtures/relay/ca-001.json` before editing. Key points:
