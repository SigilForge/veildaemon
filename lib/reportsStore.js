const memoryReports = [];
const memoryAnomalyLogs = [];
const REPORTS_KEY = "veildaemon:after-action-reports";
const ANOMALY_LOGS_KEY = "veildaemon:volunteered-anomaly-logs";
...
async function readAnomalyLogs() {
  if (reportBackend() === "upstash") {
    const stored = await redisCommand(["GET", ANOMALY_LOGS_KEY]);
    return stored ? JSON.parse(stored) : [];
  }

  return memoryAnomalyLogs;
}

async function writeAnomalyLogs(logs) {
  const nextLogs = logs.slice(0, MAX_REPORTS);
  if (reportBackend() === "upstash") {
    await redisCommand(["SET", ANOMALY_LOGS_KEY, JSON.stringify(nextLogs)]);
    return;
  }

  memoryAnomalyLogs.splice(0, memoryAnomalyLogs.length, ...nextLogs);
}
...
function makeAnomalyLogId(existing) {
  const nextNumber = existing.length + 1;
  return `ANOM-${String(nextNumber).padStart(3, "0")}`;
}
...
function createReport(input, existing) {
  ...
  const consentPublic = publicDraftApproved;
...
function createAnomalyLog(input, existing) {
  const body = input && typeof input === "object" ? input : {};
  const now = new Date().toISOString();

  return {
    id: makeAnomalyLogId(existing),
    createdAt: now,
    updatedAt: now,
    status: "under_review",
    operatorName: safeString(body.operatorName, 120),
    designation: safeString(body.designation, 120),
    primaryFrequency: safeString(body.primaryFrequency, 80),
    classification: safeString(body.classification, 120),
    attentionState: safeString(body.attentionState, 80),
    activeNeedlepoint: normalizeNeedlepoint(body.activeNeedlepoint),
    scene: safeString(body.scene, 160),
    detail: safeString(body.detail, 3000),
    severity: safeChoice(body.severity, ["trace", "minor", "active", "breach"], "trace"),
    tags: safeString(body.tags, 180),
...
function buildStats(reports) {
  const approved = reports.filter((report) => report.status === "approved" && report.publicDraftApproved).length;
  return {
    recoveredReports: reports.length,
    approvedForPublication: approved,
    containmentRestricted: reports.filter((report) => report.status === "rejected" && report.publicDraftApproved).length,
    underReview: reports.filter((report) => report.status === "under_review").length,
    redactedBeyondRecovery: reports.filter((report) => report.status === "rejected" && !report.publicDraftApproved).length,
  };
}
