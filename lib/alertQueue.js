...
function createAlert(input) {
  ...
  // Preserve native identities for Forge stream-bridge idempotency (4F.2).
  if (alert.twitchMessageId) record.twitchMessageId = String(alert.twitchMessageId);
  if (alert.source) record.source = String(alert.source);
  if (alert.soundKey) record.soundKey = String(alert.soundKey);
  return record;
}
...
async function enqueue(alert) {
  ...
  // Stage 4F.2: optional Forge stream-bridge notify (fail-open, never blocks alerts).
  // Existing overlay queue remains source of truth for presentation when Forge is down.
  try {
    const forgeBridge = require("./forgeStreamBridge");
    const forge = await forgeBridge.notifyForgeFromAlert(record);
    if (forge && forge.bridge === "ok") {
      record.forge = {
        status: forge.status || "accepted",
        eventId: forge.event_id || null,
        overlayCommands: Array.isArray(forge.overlay_commands) ? forge.overlay_commands : [],
        // Privileged actions stay proposals — do not execute here.
        pendingActions: Array.isArray(forge.pending_actions) ? forge.pending_actions : [],
      };
      // Prefer Forge presentation payload when it returned exactly one overlay command.
      const cmds = record.forge.overlayCommands;
      if (cmds.length === 1 && cmds[0] && cmds[0].payload) {
        record.forgeOverlay = {
          kind: cmds[0].kind || "alert",
          commandId: cmds[0].command_id || "",
          payload: cmds[0].payload,
        };
      }
    } else if (forge) {
      record.forge = {
        status: forge.status || forge.bridge || "skipped",
        error: forge.error || forge.reason || null,
        eventId: forge.event_id || null,
      };
    }
  } catch (error) {
    record.forge = {
      status: "error",
      error: error && error.message ? String(error.message) : "forge_bridge_failed",
    };
  }
  ...
  if (queueBackend() === "upstash") {
    ...
    return {
      alert: record,
      diagnostics: await queueDiagnostics({ pushed: true, forge: record.forge || null }),
    };
  }
